...
const API_URL = process.env.NODE_ENV === 'production' 
  ? 'https://digitext-backend.onrender.com/api'  // Your deployed backend URL
...
export const metaApi = {
  ...
  sendInstagramMessage: async (accessToken: string, igUserId: string, recipientId: string, message: string) => {
    try {
      const response = await api.post('/meta/send-instagram-message', {
        accessToken,
        igUserId,
        recipientId,
        message
  ...
  sendMessengerMessage: async (accessToken: string, pageId: string, recipientId: string, message: string) => {
    try {
      const response = await api.post('/meta/send-messenger-message', {
        accessToken,
        pageId,
        recipientId,
        message
