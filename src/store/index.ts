...
export const useStore = create<State>((set, get) => ({
  ...
  loadUserAccounts: async (userId) => {
    try {
      // For demo purposes, we'll use mock accounts
      // In a real app, you would fetch accounts from Supabase
      ...
      if (mockAccounts.length === 0) {
        ...
        set({ accounts: [instagramAccount, whatsappAccount, messengerAccount] });
