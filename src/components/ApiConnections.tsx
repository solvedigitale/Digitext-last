  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-900">API Bağlantıları</h1>
        <button
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
          <h2 className="text-lg font-medium text-gray-900">Bağlı Hesaplar</h2>
          <p className="mt-1 text-sm text-gray-500">
            Mesajlaşma platformlarına API bağlantılarınızı yönetin
          </p>
        </div>
        ...
        <div className="divide-y divide-gray-200">
          {accounts.length > 0 ? (
              ...
              <div key={account.id} className="px-4 py-4 sm:px-6">
                ...
                <div className="mt-2 text-xs text-gray-500 grid grid-cols-1 md:grid-cols-2 gap-2">
                  {account.platform === 'instagram' && (
                    <>
                      <div>
                        <span className="font-medium">Instagram Kullanıcı ID:</span> {account.igUserId || 'Ayarlanmadı'}
                      </div>
                      <div>
                        <span className="font-medium">İşletme ID:</span> {account.businessId || 'Ayarlanmadı'}
                      </div>
                    </>
                  )}
                  ...
                  {account.platform === 'messenger' && (
                    <>
                      <div>
                        <span className="font-medium">Sayfa ID:</span> {account.pageId || 'Ayarlanmadı'}
                      </div>
                      <div>
                        <span className="font-medium">İşletme ID:</span> {account.businessId || 'Ayarlanmadı'}
                      </div>
                  ...
                  {account.platform === 'whatsapp' && (
                    <>
                      <div>
                        <span className="font-medium">Telefon Numarası ID:</span> {account.phoneNumberId || 'Ayarlanmadı'}
                      </div>
                      <div>
                        <span className="font-medium">İşletme ID:</span> {account.businessId || 'Ayarlanmadı'}
                      </div>
            ...
            <div className="px-4 py-8 text-center">
              <p className="text-gray-500">Henüz API bağlantısı yapılandırılmadı</p>
              <button
                onClick={handleAddAccount}
